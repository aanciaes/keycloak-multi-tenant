...
class KubernetesEnvironmentConfigurator(private val environment: ApplicationEnvironment) :
    EnvironmentConfigurator {

    override fun buildEnvironmentConfig(): List<Module> {
        environment.log.info("Init kubernetes environment config")
    ...
    private fun initKeycloakInterfaceConfig() = module {
        single {
            KeycloakInterfaceConfigs(
                environment.config.property("kube.keycloak.realmFolder").getString(),
                environment.config.property("kube.keycloak.keycloakHost").getString()
            )
        }
    }
    ...
    private fun initDbCore() = module {
        val dataSource: DataSource = HikariDataSource(HikariConfig().apply {
            driverClassName = environment.config.property("kube.datasource.driver").getString()
            jdbcUrl = environment.config.property("kube.datasource.jdbcUrl").getString()
            username = "user"
            password = "test"
            maximumPoolSize = 5
            isAutoCommit = true
